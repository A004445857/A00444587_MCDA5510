...
namespace ReadCsv
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var sw = OpenStream(@"D:\Project\Output.csv");
            if (sw is null)
                return;
            ...
            String sDir = @"D:\Project\Sample Data";
            ...
            foreach (string y in Directory.GetDirectories(sDir))
            {
                ...
                foreach (string m in Directory.GetDirectories(sDir))
                {
                    ...
                    foreach (string d in Directory.GetDirectories(sDir))
                    {
                                    ...
                                    for (int i = 0; i < values.Length; i++)
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            if (sub == "" || sub.Equals("\"\""))
                                            {
                                                ...
                                                //Below exception is optional, it's time consuming
                                        ...
                                        {
                                            System.IO.File.AppendAllText(@"D:\Project\logs.txt", e.Message + "\n");
                                        }
                                    }
                                    lineNumber++;
                                    if (flag == 1)
                     ...
                     //below exceptions are related file operation
                            catch (FileNotFoundException)
                            {
                                System.IO.File.AppendAllText(@"D:\Project\logs.txt", "The file or directory cannot be found.\n");
                            }
                            ...
                            {
                                System.IO.File.AppendAllText(@"D:\Project\logs.txt", "The drive specified in 'path' is invalid.\n");

                            }
                            catch (PathTooLongException)
                            {
                                System.IO.File.AppendAllText(@"D:\Project\logs.txt", "'path' exceeds the maxium supported path length.\n");

                            }
                            catch (UnauthorizedAccessException)
                            {
                                System.IO.File.AppendAllText(@"D:\Project\logs.txt", "You do not have permission to create this file.\n");

                            }
                            catch (IOException e) when ((e.HResult & 0x0000FFFF) == 32)
                            {
                                System.IO.File.AppendAllText(@"D:\Project\logs.txt", "There is a sharing violation.\n");

                            }
                            catch (IOException e) when ((e.HResult & 0x0000FFFF) == 80)
                            {
                                System.IO.File.AppendAllText(@"D:\Project\logs.txt", "The file already exists.\n");

                            }
                            catch (IOException e)
                            {
                                System.IO.File.AppendAllText(@"D:\Project\logs.txt", $"An exception occurred:\nError code: " +
            ...
            System.IO.File.AppendAllText(@"D:\Project\logs.txt", "\n\nSkipped Rows :" + empty + "\n");
            System.IO.File.AppendAllText(@"D:\Project\logs.txt", "Valid Rows :" + complete + "\n");
            System.IO.File.AppendAllText(@"D:\Project\logs.txt", "Total time of exection(milliseconds) :" + stopwatch.ElapsedMilliseconds.ToString());
        }
